...
namespace SqlBulkTools
{
	public static class BulkOperationsHelper
	{
			...
			string basePropertyName)
		{
			...
			// TODO: Better solution for converting database values 
			if (type == typeof(bool))
				type = typeof(string);
			...
			object[] values, string basePropertyName, bool isComplex)
		{
				...
				CheckForValidDataType(property.PropertyType, true))
			{
				...
				if (isComplex)
				{
					...
					values[ordinal] = GetValue(property, value);
				}
				else
				{
					values[ordinal] = GetValue(property, item);
				}
			}
		}

		private static object GetValue(PropertyInfo property, object item)
		{
